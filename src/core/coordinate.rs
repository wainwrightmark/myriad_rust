...
pub struct Coordinate<const C: usize, const R: usize>(pub u8);

static_assertions::assert_eq_size!(Coordinate<3,3>, u8);

impl<const C: usize, const R: usize> fmt::Display for Coordinate<C, R> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "({},{})", self.get_column(), self.get_row())
    }
}

impl<const C: usize, const R: usize> Coordinate<C, R> {
